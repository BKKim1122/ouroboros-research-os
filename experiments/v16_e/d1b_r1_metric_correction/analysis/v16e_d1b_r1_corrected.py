...
def intervention_metrics(model, base_out, cf_out, batch, cf_batch, patched_hidden, phase: int, state_idx: int):
    ...
    non_target = [q for q in (0, 1, 2) if q != phase]
...
def factor_analysis(root: Path, seeds: list[int], n: int, device: torch.device):
    rows = []
    contexts = ["train", "rule_ood"]
    factors = [("identity", 0, 0), ("beneficiary", 1, 1), ("concern", 2, 2)]
    for rule_mode in ["compositional"]:
        for seed in seeds:
            model, cfg = load_model(root, rule_mode, seed, device)
            for context in contexts:
              for factor_i, (factor_name, phase, relation_idx) in enumerate(factors):
                base_cpu = model_mod.make_batch(n, cfg, torch.Generator().manual_seed(4100000 + seed + factor_i + (0 if context=="train" else 100000)), context)
                ...
                rows.append({"seed": seed, "context": context, "rule_mode": rule_mode, "kind": "factor", "target": factor_name, "phase": phase, "state_idx": state_idx, "patch": "matched_relation_token", **met})
                ...
                rows.append({"seed": seed, "context": context, "rule_mode": rule_mode, "kind": "factor", "target": factor_name, "phase": phase, "state_idx": state_idx, "patch": "random_norm_relation_token", **met})
        ...
        for seed in seeds:
            ...
            for context in contexts:
              for operator_i, (operator_name, phase, canonical_rule_idx) in enumerate(operators):
                base_cpu = model_mod.make_batch(n, cfg, torch.Generator().manual_seed(6100000 + seed + operator_i + (0 if context=="train" else 100000)), context)
                ...
                met = intervention_metrics(model, base_out, cf_out, base, cf, matched, phase, state_idx)
                rows.append({"seed": seed, "context": context, "rule_mode": rule_mode, "kind": "operator", "target": operator_name, "phase": phase, "state_idx": state_idx, "patch": patch_name, **met})
                met = intervention_metrics(model, base_out, cf_out, base, cf, random_patch, phase, state_idx)
                rows.append({"seed": seed, "context": context, "rule_mode": rule_mode, "kind": "operator", "target": operator_name, "phase": phase, "state_idx": state_idx, "patch": random_name, **met})
    return pd.DataFrame(rows)
...
def main():
    ...
    out_dir = root / "analysis" / "V16-E.D1b-R1"
    ...
    summary = all_df.groupby(["context", "rule_mode", "kind", "target", "patch"], as_index=False).agg(

...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--root',required=True); ap.add_argument('--seeds',nargs='+',type=int,default=[16165,16166,16167,16168,16169]); ap.add_argument('--n',type=int,default=700)
    a=ap.parse_args(); root=Path(a.root); mod=loadmod(root/'src/v16e6_surface_transition_robustness.py'); factors=['identity','beneficiary','concern']; phase_of={'identity':0,'beneficiary':1,'concern':2}; rows=[]
    for seed in a.seeds:
        run=root/'raw/V16-E.6'/f'surface_transition_seed{seed}'; meta=json.loads((run/'metadata.json').read_text()); cfg=mod.Config(**meta['config']); torch.set_num_threads(1); model=mod.Model(cfg); model.load_state_dict(torch.load(run/'checkpoint.pt',map_location='cpu',weights_only=True)); model.eval()
    ...
    df=pd.DataFrame(rows); out=root/'analysis/V16-E.6'; out.mkdir(parents=True,exist_ok=True); df.to_csv(out/'randomcode_coalition_mediation_all.csv',index=False)
    ...
    sm.to_csv(out/'randomcode_coalition_mediation_summary.csv',index=False); print(sm.to_string(index=False))

...
def sample_entity_codes(n, gen, mode, cfg):
    """Two distinct episode-local codes.

    Training mixes Rademacher and Gaussian code families. OOD uses sparse signed
    code pairs. Equality/binding, not symbol identity, is the invariant.
    """
    D = cfg.code_dim
    if mode == "encoding_ood":
        # sparse signed codes: two different dimensions per episode
    ...
    codes = torch.where(fam[:, None, None] == 0, rad, gau)
    codes = _normalize(codes)
    # Regenerate second code when too similar.
    sim = (codes[:, 0] * codes[:, 1]).sum(-1).abs()
    tries = 0
    while (sim > 0.75).any() and tries < 8:
        bad = sim > 0.75
        k = int(bad.sum())
        repl = torch.randn(k, D, generator=gen)
        repl = _normalize(repl)
        codes[bad, 1] = repl
        sim = (codes[:, 0] * codes[:, 1]).sum(-1).abs()
        tries += 1
    return codes
...
def make_batch(n, cfg, gen, mode="train", base=None, flip=None):
    ...
    conc_code[:, 0] = torch.where(conc == 1, math.sqrt(cfg.code_dim), -math.sqrt(cfg.code_dim))
...
def train(cfg, seed, out):
    ...
    (out/"metadata.json").write_text(json.dumps(dict(version="V16-E.6", seed=seed, config=asdict(cfg), finished_utc=datetime.now(timezone.utc).isoformat()), indent=2))
    return m
...
def main():
    ...
    a = ap.parse_args(); root = Path(a.root); cfg = Config(steps=a.steps); out = root/"raw"/"V16-E.6"/f"surface_transition_seed{a.seed}"

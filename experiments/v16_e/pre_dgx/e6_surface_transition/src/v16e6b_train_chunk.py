...
def main():
    ...
    a=ap.parse_args(); root=Path(a.root); mod=loadmod(root/'src/v16e6b_orthogonal_replication.py'); cfg=mod.Config(steps=a.target,threads=1,codebook_seed=a.seed); mod.seed_all(a.seed); torch.set_num_threads(1)
    out=root/'raw/V16-E.6B'/f'orthogonal_replication_seed{a.seed}'; out.mkdir(parents=True,exist_ok=True); state_path=out/'training_state.pt'
    ...
    if end>=a.target:
        ...
        (out/'metadata.json').write_text(json.dumps(dict(version='V16-E.6B',seed=a.seed,config=mod.asdict(cfg),training_mode='chunked',finished_utc=mod.datetime.now(mod.timezone.utc).isoformat()),indent=2))
        rows=[]
        model.eval()

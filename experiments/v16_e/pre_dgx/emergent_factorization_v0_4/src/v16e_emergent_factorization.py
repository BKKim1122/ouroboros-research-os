...
class Config:
    ...
    self_weight: float = 1.10
    beneficiary_weight: float = 1.65
    cost_one: float = 0.62
    cost_both: float = 1.08
    action_logit_scale: float = 5.0
    world_weight: float = 0.80
...
class UnifiedTransformer(nn.Module):
    ...
    def __init__(self, cfg: Config, condition: str):
        ...
        self.body_emb = nn.Embedding(2, d)
    ...
    def embed(self, b: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        for j in range(2):
            x[:, 1 + j] = (
                ...
                + self.body_emb(b["entity_order"][:, j])
                + self.risk_proj(b["risk_ordered"][:, j])
                + self.neutral_proj(b["neutral_ordered"][:, j])
            )
        # Identity and beneficiary role tokens.
        x[:, 3] = (self.type_emb(torch.full((n,), 3, device=device, dtype=torch.long))
                   + self.symbol_emb(b["id_symbol"])
                   + self.body_emb(b["identity"]))
        x[:, 4] = (self.type_emb(torch.full((n,), 4, device=device, dtype=torch.long))
                   + self.symbol_emb(b["ben_symbol"])
                   + self.body_emb(b["beneficiary"]))
...
def main() -> None:
    ...
    if registry_path.exists():
        try:
            registry = pd.read_csv(registry_path).to_dict("records")
        except Exception:
            registry = []
    else:
        registry = []
